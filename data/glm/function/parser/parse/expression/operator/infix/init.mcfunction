...
execute if data storage glm:parser stack[-1].metadata{no_matches:true} unless data storage glm:parser stack[-1].metadata.operator run data modify storage glm:parser raise set value ["[Expression - Operator] Unexpected '",{"storage":"glm:parser","nbt":"current.value",interpret:true},"' expected valid operator."]