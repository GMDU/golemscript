...
execute store result score $parameters bot.interpreter run data get storage glm:interpreter evaluate.stack[-1].parameters
execute if score $parameters bot.interpreter matches 0 run data modify storage glm:interpreter evaluate.stack[-1] set from storage moxlib:api/data/get output.value
