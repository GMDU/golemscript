...
execute store result score $check glm.interpreter run data modify storage glm:interpreter helpers.search_inventory.array set from storage glm:interpreter helpers.search_inventory.target

execute if score $check glm.interpreter matches 0 run data modify storage glm:interpreter helpers.search_inventory.output set from storage glm:interpreter helpers.search_inventory.current
execute if score $check glm.interpreter matches 0 store result storage glm:interpreter helpers.search_inventory.output.slot byte 1 run scoreboard players get $slot glm.interpreter
