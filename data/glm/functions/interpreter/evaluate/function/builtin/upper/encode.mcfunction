...
execute unless score $char bot.interpreter matches 97..122 run data modify storage glm:api/interpreter/function execute.return.value append from storage glm:interpreter evaluate.stack[-1].args[0].value[0]
execute unless score $char bot.interpreter matches 97..122 run return -1

execute store result storage moxlib:api/string/ascii/decode target int 1 run scoreboard players remove $char bot.interpreter 32
