...
execute if data storage moxlib:api/string/filter {output:true} run data modify storage glm:parser raise set value ["[Alphanumeric] Unexpected: '",{"storage":"glm:parser","nbt":"current.value",interpret:true},"', expected [A-Z,a-z]."]
