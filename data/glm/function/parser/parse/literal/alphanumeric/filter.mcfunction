...
execute if data storage glm:utils/string/filter {output:true} run data modify storage glm:parser raise set value '["[Alphanumeric] Unexpected: \'",{"storage":"glm:parser","nbt":"current.value"},"\', expected [A-Z,a-z]."]'
