execute unless data storage glm:interpreter evaluate.stack[-1].b{type:"literal",variant:"string"} run data modify storage glm:interpreter error set value ["TypeError: Literal string cannot be coerced to ",{"storage":"glm:interpreter","nbt":"evaluate.stack[-1].b.variant",interpret:true},"."]
