...
execute if data storage glm:parser current{value:"-"} unless data storage glm:parser stack[-1].metadata{inverted: true} run function glm:parser/parse/literal/integer/invert